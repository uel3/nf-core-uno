process MAG_DEPTHS_PLOT {
    ...
    output:
    tuple val(meta), path("${meta.assembler}-${meta.binner}-${meta.id}-binDepths.heatmap_mqc.png"), emit: heatmap
    path "versions.yml", emit: versions

    script:
    """
    plot_mag_depths_log.py \
        --depths ${depths} \
        --groups ${sample_groups} \
        --out "${meta.assembler}-${meta.binner}-${meta.id}-binDepths.heatmap_mqc.png"
